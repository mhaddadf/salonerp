		...
		<div class="inputWindow">
			<form action="" method="post">
				<table>
					...
					<?php
					if($msg != "")echo '<tr><td colspan="2">'.$msg.'</td></tr>';

					$configWriteable = false;
					$sessionsWritable = false;
					$mysqlInstalled = false;
					$sqliteInstalled = false;
					if(file_put_contents("config/test", "test") !== false)
					{
					...
					if(!$mysqlInstalled || !$sqliteInstalled || !$configWriteable || !$sessionsWritable)
					{
						echo "<tr><td colspan=\"2\">Warning: ";
						if(!$mysqlInstalled)echo "PDO-MySQL driver not installed! ";
						if(!$sqliteInstalled)echo "PDO-SQLite driver not installed! ";
						if(!$configWriteable)echo "Config folder not writable! ";
