...
function showEvent(event, time, okClicked, onPaid){
	...
	objects["product"].onChange(function(value) {
		...
		custom("productSelected", { event: event, product: value });
                if(objects["customer"].getValue() != null){
                        custom("productAndCustomerSelected", { event: event, product: value, customer: objects["customer"].getValue() });
                }
	});

	objects["customer"].onChange(function(value){
		custom("customerSelected", { event: event, customer: value });
		if(objects["product"].getValue() != null){
			custom("productAndCustomerSelected", { event: event, product: objects["product"].getValue(), customer: value });
		}
	});
...
function showReport(report){
	...
	var ask = new Array();
	var w = new Window(language[report.title]);
	var table = document.createElement("table");
	w.addCustomContent(table);
	w.show();
