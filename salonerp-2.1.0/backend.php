...
if(isset($GLOBALS["sql_execute"][$_POST["what"]]))
{
	$customFunction = $GLOBALS["sql_execute"][$_POST["what"]];
	$response = $customFunction(array(
		"data" => $_POST,
		"db" => $db,
		"execute" => function($data) { execute($data); }
	));
}
else
{
	...
	case "language":
		...
		$response = array();
		foreach($stmt->fetchAll() as $row) {
			$response[$row["name"]] = utf8_encode($row["value"]);
		}
		break;
	case "settings":
