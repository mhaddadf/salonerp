...
<head>
	...
	<script src="customizations/customizations.js" type="text/javascript"></script>
	...
	<body>
		...
		function reschedule(args, event, currentDP){
			...
			if(event.data.invoicedate == null){
				...
				custom("productSelected", { event: event, product: event.data.product });
				custom("customerSelected", { event: event, customer: event.data.customer });
				custom("productAndCustomerSelected", { event: event, product: event.data.product, customer: event.data.customer });
