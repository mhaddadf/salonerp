...
$error = "";
function install(&$error, $languages)
{
	...
	if($databaseType != "MySQL" && $databaseType != "SQLite")$error .= "Invalid database type: " . $databaseType . "<br />";
	if($password != $repeatPassword)$error .= "The passwords don't match.<br />";
	if($error != "")return;

	$configFile = fopen("config/config.php", "w") or $error .= "Can't create config file. Please check your permissions.";
	if($error != "")return;
	...
	if(!$db)
	{
		$error = "Unable to connect to database server<br />";
	...
	if(isset($logo))
	{
		...
		{
			$error = "Unable to upload logo. Check your permissions<br />";
			unlink("config/config.php");
			unlink("config/sql.php");
			return;
		}
	...
	$error = "Installation complete. You can log in <a href=\"index.php\">here</a>";
}

if(isset($_POST["install"]))
{
	install($error, $languages);
}
		...
		<div class="inputWindow">
			<form action="" method="post">
				<table>
					...
					<?php if($error != "")echo '<tr><td colspan="2">Error: '.$error.'</td></tr>'; ?>
